...
def main(*args):
    ...
    if only_evaluation == 0:
        ...
        test_loader2 = torch.utils.data.DataLoader(dataset_Test2,
            ...
            collate_fn=skip_none_collate) # We set pin_memory to False for windows, you can set it to True for linux
        ...
        train_loader = torch.utils.data.DataLoader(dataset_Train,
            ...
            num_workers=0, pin_memory=False,collate_fn=skip_none_collate)  # We set num_workers to 0 for windows, you can set it to a higher number (max_num_worker) for linux

        val_loader2 = torch.utils.data.DataLoader(dataset_Valid2,
            batch_size=1, shuffle=False,
            num_workers=0, pin_memory=False,collate_fn=skip_none_collate) # We set pin_memory to False for windows, you can set it to True for linux
    ...
    if path_to_model_weights != "":
        ...
        load = torch.load(path_model, map_location=torch.device('cpu'))
        model.load_state_dict(load['state_dict'])

    # Optimizer, scheduler, criterion
    optimizer = torch.optim.AdamW(model.parameters(), lr=LR, betas=(0.9, 0.999), eps=1e-07, weight_decay=weight_decay, amsgrad=False)
    ...
    if only_evaluation == 0:
        ...
        trainer(train_loader, val_loader2, test_loader2, model, optimizer, scheduler, criterion, 
                best_model_path, epoch_start, model_name=model_name, path_dataset=path, max_epochs=25)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_epochs',   required=False, type=int,   default=30,     help='Maximum number of epochs' )
    ...
    if not os.path.exists(args.path):
        print(f"PATH DOES NOT EXISTS: {args.path}")
        exit()
