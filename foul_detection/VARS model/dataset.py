...
class MultiViewDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for num_view in range(len(self.clips[index])):
            ...
            #video, _, _ = read_video(self.clips[index][index_view], pts_unit="sec", output_format="THWC")
            video, _, _ = read_video(
                self.clips[index][index_view],
                pts_unit="sec",
                output_format="THWC"
            )
            ...
            if final_frames is None:
                print(f"Skipping bad clip {index}: {self.clips[index][index_view]}")
                return None
            ...
            """
            if final_frames is None:
                print(f"ERROR: final_frames None for clip {index}")
